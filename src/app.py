...
from query_rag import QueryProcessor, load_session, save_session
...
def query():
    """
    Process a chat query and manage the chat session.

    This route accepts a POST request with a message and optional model and image data.
    It processes the query, updates the chat history, and saves the session state.

    Returns:
        Response: JSON object containing the response to the query.
    """
    global chat_history, session_active
    
    data = request.json
    message = data.get('message')
    model_name = data.get('model', 'ChatGPT 4o mini')  # Default to 'ChatGPT 4o mini'
    ...
    if session_active:
        ...
        chat_history = []
        session_active = True  # Enable session for follow-up questions

    processor = QueryProcessor(query_text=message, model_name=model_name, base64_image=image, chat_history=chat_history)
    processor.process_query()

    # Update the chat history and save the session
    chat_history = processor.chat_history
...
def read_aloud():
    ...
    try:
        ...
        return Response(stream_with_context(generate_audio_stream()), mimetype='application/octet-stream')
